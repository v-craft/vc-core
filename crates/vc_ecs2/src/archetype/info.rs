...
impl Archetype {
    pub(crate) unsafe fn get_storage_info_unchecked(
        &self,
        component_id: ComponentId,
    ) -> (StorageType, StorageIndex) {
...
impl Debug for Archetypes {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_tuple("Archetypes").field(&self.archetypes).finish()
    }
}
...
impl Archetypes {
    ...
    pub(crate) fn register(
        ...
        let id = NonMaxU32::new(self.archetypes.len() as u32)
            .unwrap_or_else(|| panic!("too many archetypes"));
    ...
    pub fn set_bundle_map(&mut self, bundle_id: BundleId, archetype_id: ArchetypeId) {
