...
use crate::component::{Component, ComponentId, NoSendResource, Resource};
...
impl ComponentDescriptor {
    ...
    // TODO: Mark as `const fn` when `type_name` is const fn.
    #[inline]
    pub fn new_no_send<T: NoSendResource>() -> Self {
        Self {
            type_id: TypeId::of::<T>(),
            layout: Layout::new::<T>(),
            kind: ComponentKind::NoSendResource,
            mutable: T::MUTABLE,
            storage_type: StorageType::SparseSet, // unused
            clone_behavior: T::CLONE_BEHAVIOR,
            drop_fn: Self::drop_fn_for::<T>(),
...
pub struct Components {
    ...
    pub(crate) no_sends: TypeIdMap<ComponentId>,
}
...
impl Components {
    pub(crate) const fn new() -> Self {
        Self {
            ...
            no_sends: TypeIdMap::new(),
        }
    }

    /// Returns a reference to an `ComponentInfo` depending on the `ComponentId`.
    #[inline]
    ...
    #[inline]
    pub fn get_no_send_id(&self, type_id: TypeId) -> Option<ComponentId> {
        self.no_sends.get(&type_id).copied()
    }
    ...
    #[inline(always)]
    pub fn contains_no_send(&self, type_id: TypeId) -> bool {
        self.no_sends.contains(&type_id)
    }
}
