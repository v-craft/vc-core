...
impl BlobArray {
        ...
        if current_capacity != 0 {
            unsafe {
                if let Some(drop_fn) = self.drop_fn {
                    ...
                    (0..len).for_each(|index| drop_fn(self.get_mut(index).promote()));
    ...
    pub const unsafe fn get(&self, index: usize) -> Ptr<'_> {
        let size = self.item_layout.size();
        unsafe { Ptr::new(self.data.add(size * index)) }
    }
    ...
    pub const unsafe fn get_mut(&mut self, index: usize) -> PtrMut<'_> {
        let size = self.item_layout.size();
        unsafe { PtrMut::new(self.data.add(size * index)) }
    }
    ...
    #[inline(always)]
    #[must_use = "The returned pointer should be used to drop the removed element"]
    pub const unsafe fn swap_remove_not_last(
        &mut self,
        index: usize,
        last_index: usize,
    ) -> OwningPtr<'_> {
