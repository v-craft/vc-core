...
use crate::{component::ComponentId, utils::DebugCheckedUnwrap};
...
impl ComponentIndices {
    ...
    #[inline]
    pub unsafe fn get_unchecked(&self, id: ComponentId) -> u32 {
        unsafe {
            let &item = self.indices.get(id.index()).debug_checked_unwrap();
            item.debug_checked_unwrap().get()
        }
    }
